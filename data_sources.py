...
TRACKED_TICKERS = [
    "SPY", "QQQ", "IWM", "XLE", "XLF", "XLV", "GLD", "TLT",
    "ESTC", "NVDA", "MSFT", "AMZN",
]
...
def build_trade_ideas() -> pd.DataFrame:
    ...
    if not rows:
        return pd.DataFrame(
            columns=[
                "Ticker", "Name", "Price", "1M %", "3M %",
                "Signal", "Action", "Conviction", "Score",
            ]
        )
...
def executive_brief(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if market_tone == "risk-off":
        ...
        game_plan.append(
            "Lean into growth conversations where the business case is strong."
        )
    ...
    if not ideas_df.empty:
        ...
        if pd.notna(top["1M %"]) and top["1M %"] > 5:
            ...
            game_plan.append(
                "No standout momentum setup today, so patience is the better move."
            )
    else:
        game_plan.append(
            "No strong trading setups today. Preserve capital and wait for cleaner signals."
        )

    if not data["world_df"].empty:
        game_plan.append(
            "Keep an eye on geopolitical headlines because they can shift sentiment quickly."
        )

    try:
        if float(data["us_cpi"].get("value", 0)) > 3:
            game_plan.append(
                "Stay aware of cost pressure and spending caution across the market."
            )
    except Exception:
        pass
